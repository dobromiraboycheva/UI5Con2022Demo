...
sap.ui.define([
	 ...
	 * @private
	 */
	SmartTable.prototype._createColumnMenu = function(oColumn, bResizeButton) {
		var oResourceBundle = Core.getLibraryResourceBundle("sap.ui.comp");
		var oColumnInfo = oColumn.data("p13nData");

		this._oFilterButton = new ActionItem({
			label: oResourceBundle.getText("SMARTTABLE_RP_FILTER"),
			icon: "sap-icon://filter",
			press: [this._onCustomFilter, this]
		});
		var oQuickSort = new QuickSort({
			items: [new QuickSortItem({key: oColumnInfo.columnKey, label: this._getColumnLabel(this._oColumnClicked)})],
			change: [
				function (oEvent) {
					var oSortOrder = oEvent.getParameter("item").getSortOrder();
					if (oSortOrder == SortOrder.None) {
						var oColumnInfo = this._oColumnClicked.data("p13nData");
						this._onCustomSort(oEvent, oColumnInfo.sorted && oColumnInfo.sorted.ascending);
					} else {
						this._onCustomSort(oEvent, oSortOrder == SortOrder.Ascending);
					}
				}, this
			]
		});
		this._aSortButton = oQuickSort.getEffectiveQuickActions();

		this._oColumnHeaderPopover = new ColumnMenu();
		this._oColumnHeaderPopover.addAggregation("_quickActions", oQuickSort);
		this._oColumnHeaderPopover.addAggregation("_items", new ActionItem({
			label: oResourceBundle.getText("SMARTTABLE_RP_SORT"),
			icon: "sap-icon://sort",
			press: [this._showTableSortDialog, this]
		}));
		this._oColumnHeaderPopover.addAggregation("_items", this._oFilterButton);

		if (bResizeButton) {
			this._oColumnHeaderPopover.addAggregation(
				"_quickActions",
				new QuickAction({
	...
	SmartTable.prototype._onColumnPress = function(oEvent) {
		...
		if (!this._oColumnHeaderPopover) {
			if (this._bUseColumnMenu) {
				this._createColumnMenu(this._oColumnClicked, bResizeButton);
			} else {
