...
// }
	...
	FilterBar.prototype.setBasicSearch = function(oBasicSearchField) {
		...
		if (oBasicSearchField && !this.getDisableSearchMatchesPatternWarning()) {
			this._basicSearchValidationEnabled.call(this, oBasicSearchField);
		}
...
//			}
	...
	/**
	 * Called when basic search is changed and validates the symbols passing the search pattern
	 * @private
	...
	FilterBar.prototype._onLiveChangeValidateSymbols = function(oEvent) {
		var rValidateSymbolAsteriskRegex = new RegExp("[\\w][*][\\w]"),
			...
			oValueStateMessage = oControl._oValueStateMessage,
			bContainSpecialSymbol = rValidateSymbolAsteriskRegex.test(oControl.getValue()) || rValidateSymbolPlusRegex.test(oControl.getValue());

		if (bContainSpecialSymbol && !oControl.hasStyleClass("sapMSFContentWrapperWarning")) {
			oControl.addStyleClass("sapMSFContentWrapperWarning");
			oControl.getValueState = function () {
				return ValueState.Warning;
			};

			oValueStateMessage.open();
		} else if (!bContainSpecialSymbol && oControl.hasStyleClass("sapMSFContentWrapperWarning")) {
			oControl.getValueState = function () {
				return ValueState.None;
			};

			oValueStateMessage.close();
			oControl.removeStyleClass("sapMSFContentWrapperWarning");
		}
	};

	/**
	 * Adds function implementation to basic search and attaches live change validation to the search pattern
	 * @private
	 * @param {object} oBasicSearchControl Basic seach field
	 */
	FilterBar.prototype._basicSearchValidationEnabled = function(oBasicSearchControl) {
		oBasicSearchControl.getValueState = function() {
			return ValueState.None;
		};

		oBasicSearchControl.getValueStateText = function() {
			return sap.ui.getCore().getLibraryResourceBundle("sap.ui.comp").getText("SEARCH_FIELD_VALIDATION_WARNING_MESSAGE");
		};

		var oValueStateMessage = new ValueStateMessage(oBasicSearchControl);
		oBasicSearchControl._oValueStateMessage = oValueStateMessage;
		oBasicSearchControl.getDomRefForValueStateMessage = function(){
			return oBasicSearchControl.getDomRef();
		};

		var fnOnFocusInOriginal = oBasicSearchControl.onfocusin;
		oBasicSearchControl.onfocusin = function () {
			fnOnFocusInOriginal && fnOnFocusInOriginal.apply(this, arguments);
			oValueStateMessage.open();
		};

		var fnOnFocusOutOriginal = oBasicSearchControl.onfocusout;
		oBasicSearchControl.onfocusout = function () {
			fnOnFocusOutOriginal && fnOnFocusOutOriginal.apply(this, arguments);
			oValueStateMessage.close();
		};

		var fnOnExit = oBasicSearchControl.exit;
		oBasicSearchControl.exit = function () {
			fnOnExit && fnOnExit.apply(this, arguments);
		...
		oBasicSearchControl.attachLiveChange(this._onLiveChangeValidateSymbols.bind(this));
	};
