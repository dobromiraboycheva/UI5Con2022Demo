...
sap.ui.define([
	...
	function isNewSourceElementOnCombineRequired(oChangeDefinition) {
		return !!oChangeDefinition.content.newElementId;
	}

	function createNewGroupElement(mPropertyBag, oChangeDefinition, oParent) {
		var oModifier = mPropertyBag.modifier;
		var oAppComponent = mPropertyBag.appComponent;
		var oView = mPropertyBag.view;

		if (isNewSourceElementOnCombineRequired(oChangeDefinition)) {
			return oModifier.getAggregation(oParent, "dependents")
				.then(function (aDependentGroupElements) {
					var oDependentControl = aDependentGroupElements.find(function (oElement) {
						return oModifier.bySelector(oChangeDefinition.content.newElementId, oAppComponent, oView);
					});
					...
					return oModifier.createControl(
						...
						mPropertyBag.view,
						oChangeDefinition.content.newElementId
					);
				});
		}
		// old combine change handler handling, where the sourceControl was not replaced by a new one
		return Promise.resolve(oModifier.bySelector(oChangeDefinition.content.sourceSelector, oAppComponent, oView));
	}
	...
	function combineSingleFields(oGroupElement, oSourceControl, oModifier, oView, oChangeDefinition, i, aSingleFields) {
		if (oChangeDefinition.content.newElementId || oGroupElement !== oSourceControl) {
	...
	CombineFields.applyChange = function(oChange, oControl, mPropertyBag) {
		...
		return createNewGroupElement(mPropertyBag, oChangeDefinition, oParent)
			...
			return aGroupElements.reduce(function (previousPromise, oGroupElement, iIndex) {
				return previousPromise.then(function (iFieldsAdded) {
					...
					if (oText && oText.value !== sPreviousLabel && oText.value.length > 0) {
						if (bIsRtl) {
							aLabelText.unshift(oText.value);
						} else {
							aLabelText.push(oText.value);
						}
				...
				.then(function (oAggregation) {
					if (oAggregation.indexOf(oGroupElement) === -1) {
						...
						return Promise.resolve()
							...
							.then(function () {
								if (isNewSourceElementOnCombineRequired(oChange.getDefinition())) {
	...
	CombineFields.completeChangeContent = function(oChange, oSpecificChangeInfo, mPropertyBag) {
		...
		if (oSpecificChangeInfo.newElementId) {
			oChangeDefinition.content.newElementId = oSpecificChangeInfo.newElementId;
		}
