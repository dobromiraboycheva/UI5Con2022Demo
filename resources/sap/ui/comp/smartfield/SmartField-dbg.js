...
sap.ui.define([
	...
	SmartField.prototype.init = function() {
		...
		this.attachModelContextChange(function () {
			...
			// and the default model is not ppropageted to the SmartField
			// we do nothing
	...
	SmartField.prototype.setTextInEditModeSource = function(sTextInEditModeSource) {
		...
		if (sTextInEditModeSource !== sOldTextInEditModeSource) {
			...
			if (this._oFactory) {
				this._oFactory.bind({mode: this.getMode(), rebind: true});
			}
	...
	SmartField.prototype._setOnInnerControl = function() {
		var sJustifyContent, oChild = this.getContent();
	...
	SmartField.prototype._onInnerControlToggled = function(oSettings) {
		...
		if (oSettings.mode === "edit" && this.getContent()) {
	...
	SmartField.prototype._updateInnerControlsIfRequired = function() {
		...
		if (oUomSmartField && oUomSmartField.isA("sap.ui.comp.smartfield.SmartField")) {
			oUomSmartField._updateInnerControlsIfRequiredHandler(oControlSettings);
		}
	};
	...
	SmartField.prototype._createControlIfRequired = function(oSettings) {
		...
		if ((sMode !== this._oControl.current) || !vControlForCurrentMode) {
			...
			this._setOnInnerControl();
			oEmbeddedSmartField = this._getEmbeddedSmartField();
	...
	SmartField.prototype._isValueInitial = function () {
		...
		if (!oDataState) {
			this._bValueNotInitial = true; // Value no longer considered initial
			return true;
		}

		if (oDataState.getValue() !== oDataState.getOriginalValue()) {
			this._bValueNotInitial = true; // Value no longer considered initial
			return false;
		}
	...
	/**
	 * A protected method that checks if the <code>SmartField</code> has a <code>BindingContext</code>
	 * and whether this <code>BindingContext</code> points to the default data model.
	 *
	 * @returns {boolean}
	 * @private
	 */
	SmartField.prototype._isBoundToDefaultModel = function () {
		...
		return bHasDefaultModel && oBindingContext !== undefined && oBindingContext !== null;
	};
	...
	SmartField.prototype._initState = function () {
		return Object.create({
			...
			isEntitySetChanged: function (sEntitySet) {
				var sKey = "ENTITY_SET",
					bHasChanged = (typeof sEntitySet !== "undefined" && sEntitySet !== "" && this._getProperty(sKey) !== sEntitySet);
